...
boolean isMLXOBJ = false;
boolean isMLXAMB = false;
boolean isGYROX = false;
boolean isGYROY = false;
boolean isGYROZ = false;
...
//    lcd.print(device);
    ...
    */
        ...
        else if (device == MLXOBJ)
        {
          isMLXOBJ = true;
        }
        else if (device == MLXAMB)
        {
          isMLXAMB = true;
        }
        else if (device == GYROX)
        {
          isGYROX = true;
        }
        else if (device == GYROY)
        {
          isGYROY = true;
        }
        else if (device == GYROZ)
        {
          isGYROZ = true;
        }
...
void runSet(int device)
{
    ...
    case RGBLED: //RGBLED
    {
      int port1 = readBuffer(7);
      int port2 = readBuffer(9);
      int port3 = readBuffer(11);
      int value1 = readBuffer(13);
      int value2 = readBuffer(15);
      int value3 = readBuffer(17);

      setPortWritable(port1);
      setPortWritable(port2);
      setPortWritable(port3);

      if (port1 == 0 && port2 == 0 && port3 == 0 && value1 == 0 && value2 == 0 && value3 == 0) {
        analogWrite(port1, 0);
        analogWrite(port2, 0);
        analogWrite(port3, 0);
      }
...
void sendMLX()
{
  Adafruit_MLX90614 mlx = Adafruit_MLX90614();
  mlx.begin();
  float fObjC = mlx.readObjectTempC();
  float fAmbC = mlx.readAmbientTempC();
...
Wire.begin();
  ...
  if (isGYROX) {
    writeHead();
    sendFloat(mpu6050.getAngleX());
    writeSerial(GYROX);
    writeEnd();
  }
  if (isGYROY) {
    writeHead();
    sendFloat(mpu6050.getAngleY());
    writeSerial(GYROY);
    writeEnd();
  }
  if (isGYROZ) {
    writeHead();
    sendFloat(mpu6050.getAngleZ());
    writeSerial(GYROZ);
    writeEnd();
  }
}
